...
#     a1x = 6.4 + 2
#     s1x = 78.7 + 110
#     oneplane(a10, s10, u11, s1, a11, s11, a1x, s1x)
#
#
# def twoplane_test():
#     a10 = 2
...
def oneplane(a10, s10, u11, s1, a11, s11, a1x, s1x):
    ...
    # return [Vector(*complex2magnitude_phase(u1))], [v_factor]
    return [Vector(*complex2magnitude_phase(u1))], [factor]
...
def twoplane(
    ...
    try:
        ...
        return [Vector(0, 0) for _ in range(2)], [Vector(0, 0) for _ in range(4)]
    # return [Vector(*complex2magnitude_phase(u1)), Vector(*complex2magnitude_phase(u2))], v_factor
    return [Vector(*complex2magnitude_phase(u1)), Vector(*complex2magnitude_phase(u2))], [k11, k21, k12, k22]
...
# @time_cal
def process_v(ctx: Ctx) -> list[Vector]:
    if ctx.pipeline == 0:
        if ctx.init_finished == False:
            args = {
                'a10': ctx.response_1_1.length,
                's10': ctx.response_1_1.angle,
                'u11': ctx.test_weight_1.weight,
                ...
                'a11': ctx.response_1_2.length,
                's11': ctx.response_1_2.angle,
                'a1x': ctx.response_1_1.length,
                's1x': ctx.response_1_1.angle,
            }
            res = oneplane(**args)
            ...
            factor = res[1][0]
            # 将影响系数计入ctx
            ctx.factor_1_1 = factor
            return res[0]

        else:
            args = {
                'a1x': ctx.response_1_0.length,
                's1x': ctx.response_1_0.angle,
            }
            v0 = scalar2vector(**args)

            factor = ctx.factor_1_1

            u1 = v0 / factor
            return [Vector(*complex2magnitude_phase(u1))]
        ...
        if ctx.init_finished == False:
            args = {
                ...
                'a1x': ctx.response_1_1.length,
                's1x': ctx.response_1_1.angle,
                'a2x': ctx.response_2_1.length,
                's2x': ctx.response_2_1.angle,
            }
            res = twoplane(**args)
            # 从算法返回值中取得影响系数
            k11 = res[1][0]
            ...
            args_2 = {
                'a2x': ctx.response_2_0.length,
                's2x': ctx.response_2_0.angle,
            }

            v0 = scalar2vector(**args_1)
            v1 = scalar2vector(**args_2)
            ...
            u1 = k22 / delta * v0 - k12 / delta * v1
            u2 = k11 / delta * v1 - k21 / delta * v0
        return [Vector(*complex2magnitude_phase(u1)), Vector(*complex2magnitude_phase(u2))]
